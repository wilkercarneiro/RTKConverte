...
async function testPdfIntegrity() {
  ...
  console.log("--- TESTANDO PLANTA COM ESPÓLIO ---");
  const dadosEspolio = {
    ...
    denominacao: "FAZENDA VIBRAÇÃO - Parte 1",
    proprietarios: [{
    ...
    trt: "BR20260408910",
    rt: { nome: "DANIEL NASCIMENTO SANTOS", formacao: "Técnico em Agropecuária", conselhoSigla: "CFTA", conselhoNumero: "0578839458-9", codigoCredenciado: "DSBN" },
    desenhista: "JANETE OLIVEIRA", dataStr: "24/07/2026",
    logo: null, satelite: null,
  };

  const pdfBytes = await gerarPlantaPdf(dadosEspolio);
  console.log("PDF gerado, tamanho:", pdfBytes.length, "bytes");

  // Validando se o PDF pode ser recarregado sem corrupção
  const pdfDoc = await PDFDocument.load(pdfBytes);
  console.log("PDF recarregado com sucesso! Páginas:", pdfDoc.getPageCount());
}

testPdfIntegrity().catch(console.error);
